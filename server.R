...
server <- function(input, output, session) {
    ...
    parse_ints <- function(s) {
        ...
        is_int <- grepl("^[-+]?[0-9]+$", toks)
        vals <- as.integer(toks[is_int])
        is_bad <- !is_int | vals < 0 | vals == 1
        list(vals = vals[vals >= 0 & vals != 1], bad = toks[is_bad])
    }
    ...
    output$summary <- renderText({
        ...
        paste("Wahrscheinlichkeit, den Test nicht zu bestehen (%, 90%-Konfidenzintervall)",paste(round(100*(1-d["10",c(3,1)]),2), collapse=", "))
    })
